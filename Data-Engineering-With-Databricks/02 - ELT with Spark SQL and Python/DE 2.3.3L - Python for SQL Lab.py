...
# MAGIC %run ../Includes/classroom-setup-2.3.3L
...
# MAGIC 
# MAGIC Look at the following 2 cells. Before executing each cell, identify:
...
display(spark.sql(f"SELECT '{course}' AS course_name"))
...
db_name = f"dbacademy_{clean_username}_{course}"
...
# MAGIC %md
# MAGIC Below, we add a simple control flow statement to this logic to create and use this user-specific database. Optionally, we will reset this database and drop all of the contents on repeat execution. (Note the the default value for the parameter **`reset`** is **`True`**).
