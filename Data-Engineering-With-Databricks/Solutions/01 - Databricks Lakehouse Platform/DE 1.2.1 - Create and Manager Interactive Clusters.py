...
# MAGIC 
# MAGIC The specifics of how clusters are configured varies slightly between cloud vendors, but at a high level, it's safe to generalize that a Databricks cluster is an ephemeral (temporary, short-lived) collection of cloud resources used to execute arbitrary code. At its most simple, a cluster comprises of:
...
# MAGIC * Give the cluster a name you'll remember, for example `<your_initials>_dewd`
...
# MAGIC * For node type, select the smallest type that provide 4 cores and advertises that it is **Delta Cache Accelerated**
# MAGIC * Click the blue **Create Cluster** button
# MAGIC 
