...
-- MAGIC WHEN MATCHED THEN UPDATE SET a.email = b.email
-- MAGIC WHEN NOT MATCHED THEN INSERT *
-- MAGIC ```
-- MAGIC 
-- MAGIC As we implemented the `users` table as a Type 1 SCD Delta table with an `updated` field, we can leverage this field while performing a merge operation. Let's make sure that records that are updated OR inserted have the same timestamp. This operation will be completed as a single batch to avoid potentially leaving our table in a corrupt state.
...
-- MAGIC 
-- MAGIC When merging new events parsed from `events_raw`, confirm that an identical record isn't already in the `events_clean` table.
-- MAGIC ```
-- MAGIC MERGE INTO target a
-- MAGIC USING source b
-- MAGIC ON a.data = b.data
