...
AS SELECT <FILL-IN add columns receipt_time and dataset in addition to all columns from source>
  FROM cloud_files("<CLOUD_FILES LOCATION>", "json")
...
CREATE <FILL-IN> pii
AS SELECT *
  FROM cloud_files("/mnt/training/healthcare/patient", "csv", map(<FILL-IN specify parameters>))
...
-- TODO
CREATE <FILL-IN> recordings_parsed
AS SELECT 
...
CREATE INCREMENTAL LIVE TABLE recordings_enriched
  (<FILL-IN add a contraint to drop records when heartrate ! > 0>)
